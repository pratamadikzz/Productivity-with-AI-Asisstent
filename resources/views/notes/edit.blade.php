            ...
            <form action="{{ route('notes.update', $note) }}" method="POST"
                ...
                <div class="mt-6">
                    ...
                    <select id="category_id" name="category_id"
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" @selected(old('category_id', $note->category_id) == $category->id)>
                                {{ $category->name }}
                            </option>
                        @endforeach
                ...
                <div class="mt-6">
                    ...
                    <textarea id="content" name="content" rows="16"
                        class="mt-2 w-full rounded-lg border border-slate-200 px-4 py-3 text-sm leading-6 outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200">{{ old('content', $note->content) }}</textarea>
